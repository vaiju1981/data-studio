...
bank where six questions carry a value proved separately in SQL and the rest
assert only that an answer arrived with a query behind it. Both are worth running
...
def test_the_single_csv_bank_is_mostly_unanchored() -> None:
    ...
    Thirty-two of its questions asserted only that an answer arrived, was not a
